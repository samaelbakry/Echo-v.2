...
const FriendProfile = () => {
  ...
  const { data: friendData } = useQuery({
    queryKey: ["getFriendData", userProfileId],
  ...
  const { data: friendPosts, isLoading } = useQuery({
    queryKey: ["getFriendPosts", userProfileId],
  ...
  return (
    <>
      {friendData && (
        <Helmet>
          <title>{friendData.name} | Profile</title>
        </Helmet>
      )}
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 my-5 p-5 gap-5">
        <div className="my-1 p-2 col-span-1 bg-blur self-start">
          <div className="h-24 rounded-md relative ">
            <img
              src={friendData?.cover || fallBackImg}
              alt="cover"
          ...
          <div className="mt-12 px-4 flex flex-col gap-1">
            <h2 className="text-lg font-semibold dark:text-white/80">
              {friendData?.name}
            </h2>
            ...
            <span className="text-sm text-gray-600 dark:text-white/80">
              Birth date:{" "}
              {new Date(friendData?.dateOfBirth).toLocaleDateString()}
            </span>
